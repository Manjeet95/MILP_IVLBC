...
python Ineq_Reduction.py IVLBC sbox GUROBI -
python Ineq_Reduction.py IVLBC prob GUROBI -
python Ineq_Reduction.py IVLBC sbox GUROBI 0 2 
python Ineq_Reduction.py IVLBC sbox_2 GUROBI -
python Ineq_Reduction.py IVLBC prob CPLEX 10 2
"""
...
def compute_polytope_halfspaces(vertices):
    ...
    for i in range(0,len(bA)):
        mask = np.array([float(val).is_integer() for val in bA[i]])
        mul_factor = 2
        while(mask.sum()!=V.shape[1]+1):
            bA[i] = mul_factor*bA[i]
            mul_factor += 1
            mask = np.array([float(val).is_integer() for val in bA[i]])
    if bA.shape == (0,):  # bA == []
        return bA
...
if (sys.argv[1] == "LICID"):
  ...
  s_box = ((0x6 , 0x5 , 0xC , 0xA , 0x1 , 0xE , 0x7 , 0x9 , 0xB , 0x0 , 0x3 , 0xD , 0x8 , 0xF , 0x4, 0x2),);     # ULC				
    ...
    s_box = ((0xF , 0x8 , 0x3 , 0xE , 0x0 , 0x7 , 0xB , 0xA , 0x5 , 0xD , 0x9 , 0xC, 0x6 , 0x4, 0x2, 0x1),);     # SLA	
